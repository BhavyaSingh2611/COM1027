...
public class Tenant implements ITenant {
     ...
     * @throws IllegalArgumentException if the forename or surname does not
     *                                  start with a capital letter or if the
     *                                  age is less than 17
    ...
    public Tenant(String forename, String surname, int age,
        ...
        if (!this.validateName(forename) || !this.validateName(surname)) {
            throw new IllegalArgumentException("Forename and surname"
                    + " must start with a capital letter");
        }
        if (age <= 16) {
            throw new IllegalArgumentException("Tenant must be "
